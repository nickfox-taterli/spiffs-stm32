...
/**
  ...
  * @retval None
  */
void BSP_QSPI_Read(uint32_t ReadAddr, uint32_t Size,uint8_t *pData)
{
    QSPI_CommandTypeDef sCommand;

		SEGGER_RTT_printf(" > BSP Read = 0x%08X,Size = %d.\r\n",ReadAddr,Size);
...
/**
  ...
  * @retval None
  */
void BSP_QSPI_Write(uint32_t WriteAddr, uint32_t Size,uint8_t *pData)
{
    QSPI_CommandTypeDef sCommand;
    uint32_t end_addr, current_size, current_addr;

		SEGGER_RTT_printf(" > BSP Write = 0x%08X,Size = %d.\r\n",WriteAddr,Size);	
...
void BSP_QSPI_Erase_Block(uint32_t BlockAddress)
{
    QSPI_CommandTypeDef sCommand;
		...
		SEGGER_RTT_printf(" > BSP Erase = 0x%08X.\r\n",BlockAddress);		
...
void BSP_QSPI_Erase_Chip(void)
{
    ...
    QSPI_Command(&sCommand);
		...
		SEGGER_RTT_printf(0," > BSP Erase Chip.\r\n");	
    /* Configure automatic polling mode to wait for end of erase */
    BSP_QSPI_AutoPollingMemReady();
}
